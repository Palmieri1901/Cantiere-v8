...
def _build_preventivo_pdf(p: PreventivoTubolare, cfg: TubolariConfig, cantiere: dict) -> bytes:
    ...
    listino_rows = [
        ...
        [Paragraph("Tessuto ORCA (differenza di prezzo da aggiungere in caso di scelta)", st_row),
         Paragraph(_fmt_eur(orca_totale), st_row_bold), ""],
        # Lavorazioni
        [Paragraph("<b>LAVORAZIONI EXTRA</b>", st_row_bold), "", ""],
        [Paragraph("A) Rifinitura interna strisciato (differenza di prezzo da aggiungere in caso di scelta)", st_row),
         Paragraph(_fmt_eur(rif_totale), st_row_bold), ""],
        [Paragraph("B) Bottazzo doppio h 90 mm (differenza di prezzo da aggiungere in caso di scelta)", st_row),
