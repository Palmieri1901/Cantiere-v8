...
// ============================================================================
// COMPONI ORDINE (dialog interattivo — sotto scorta + aggiunte manuali)
// ============================================================================

function ComponiOrdineDialog({ open, onOpenChange, articoli, fornitori }) {
  const [items, setItems] = React.useState([]); // { articolo_id, quantita, nota }
  const [q, setQ] = React.useState("");
  const [fFornitore, setFFornitore] = React.useState("all");
  const [note, setNote] = React.useState("");
  const [downloading, setDownloading] = React.useState(false);

  const artMap = React.useMemo(() => Object.fromEntries((articoli || []).map((a) => [a.id, a])), [articoli]);
  const fornMap = React.useMemo(() => Object.fromEntries((fornitori || []).map((f) => [f.id, f.nome])), [fornitori]);

  // Pre-popola con articoli sotto scorta all'apertura
  React.useEffect(() => {
    if (!open) return;
  ...
  const setNota = (aid, v) => setItems((prev) => prev.map((i) => i.articolo_id === aid ? { ...i, nota: v } : i));
  ...
  // Raggruppa items per fornitore per una vista più leggibile
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl" data-testid="dialog-componi-ordine">
        ...
        <div className="grid md:grid-cols-2 gap-4 max-h-[65vh]">
          {/* Colonna sinistra: articoli nell'ordine, raggruppati per fornitore */}
          ...
          {/* Colonna destra: catalogo per aggiungere */}
